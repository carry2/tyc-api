...
public class FundIndustry{
    ...
    //基金对应的为3
    private static Integer business_type = 3;
    //自动计算为1 手动添加为2
    ...
    public static void main(String[] args)
    {
        ...
        for (Record r:allBusinessIds) {
            Long business_id = r.get("fund_company_id");
            //根据投资方ID获取所有公司ID
    ...
    {
        List<Record> recordList = Db.use(DB_CMP_NAME).find("SELECT  DISTINCT(fund_company_id) fund_company_id  FROM "+DB_CMP_NAME+".tbl_event_invest where business_type='2' and fund_company_id   is not null and status='1'");
        return recordList;
    }
    ...
    public static List<Record> findAllBusinessCompanysId(Long business_id)
    {
        ...
        List<Record> recordList = Db.use(DB_CMP_NAME).find("select DISTINCT(b.company_id) company_id from  "+DB_CMP_NAME+".tbl_event_invest a LEFT JOIN   "+DB_CMP_NAME+".tbl_event b on a.event_id=b.id where a.status='1' and b.status='1' and a.fund_company_id='"+business_id+"' ");
        return recordList;
    }
